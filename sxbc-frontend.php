...
// Shortcode
function sxbc_shortcode($atts, $content = null) {
		// Set default shortcode attributes
	$options = get_option( 'sxbc_settings' );
	if(!$options){
		sxbc_set_options ();
		$options = get_option( 'sxbc_settings' );
	}
	...
	return sxbc_frontend($atts);
}
add_shortcode('sx-carousel', 'sxbc_shortcode');

// Display carousel
function sxbc_frontend($atts){
	...
	while ( $loop->have_posts() ) {
		...
		if ( '' != get_the_post_thumbnail(get_the_ID(), $atts['image_size']) ) {
			...
			$url = get_post_meta(get_the_ID(), 'sxbc_image_url', true);
			$url_openblank = get_post_meta(get_the_ID(), 'sxbc_image_url_openblank', true);
			$link_text = get_post_meta(get_the_ID(), 'sxbc_image_link_text', true);
	...
	if(count($images) > 0){
		ob_start();
		?>
		<div id="sxbc_<?php echo $id; ?>" class="carousel slide" <?php if($atts['use_javascript_animation'] == '0'){ echo ' data-ride="carousel"'; } ?> data-interval="<?php echo $atts['interval']; ?>">
			
			<?php // First content - the carousel indicators
			if( count( $images ) > 1 ){ ?>
				<ol class="carousel-indicators">
				<?php foreach ($images as $key => $image) { ?>
					<li data-target="#sxbc_<?php echo $id; ?>" data-slide-to="<?php echo $key; ?>" <?php echo $key == 0 ? 'class="active"' : ''; ?>></li>
				<?php } ?>
				</ol>
			<?php } ?>
			...
			if( count( $images ) > 1 ){
				if($atts['showcontrols'] === 'true' && $atts['twbs'] == '3') { ?>
					<a class="left carousel-control" href="#sxbc_<?php echo $id; ?>" data-slide="prev"><span class="glyphicon glyphicon-chevron-left"></span></a>
					<a class="right carousel-control" href="#sxbc_<?php echo $id; ?>" data-slide="next"><span class="glyphicon glyphicon-chevron-right"></span></a>
				<?php } else if($atts['showcontrols'] === 'true'){ ?>
					<a class="left carousel-control" href="#sxbc_<?php echo $id; ?>" data-slide="prev">‹</a>
					<a class="right carousel-control" href="#sxbc_<?php echo $id; ?>" data-slide="next">›</a>
				<?php } else if($atts['showcontrols'] === 'custom' && $atts['twbs'] == '3' &&  $atts['customprev'] != '' &&  $atts['customnext'] != ''){ ?>
					<a class="left carousel-control" href="#sxbc_<?php echo $id; ?>" data-slide="prev"><span class="<?php echo $atts['customprev'] ?> icon-prev"></span></a>
					<a class="right carousel-control" href="#sxbc_<?php echo $id; ?>" data-slide="next"><span class="<?php echo $atts['customnext'] ?> icon-next"></span></a>
				<?php }
			} ?>
		...
		<script type="text/javascript">
			jQuery(document).ready(function() {
				jQuery('#sxbc_<?php echo $id; ?>').carousel({
